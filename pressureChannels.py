...
g = G(
    print_lines=False,
    outfile=r"H:\User Files\Fitzgerald\SoftRobots\pressureChannels\gcode\pressureChannelArray.pgm",
...
mold_z_zero = -79.337
mold_depth = 12
print_height = 0 #mold_z_zero+5#0.5*mold_depth
petri_center_x = 394.742
petri_center_y = 128.601
...
def change_tool(to_tool_index):
    ...
    global g
    from_tool  = tools[cur_tool_index]
    cur_tool = tools[to_tool_index]
    g.write("; Change Tools from " + from_tool + " to " + cur_tool + ".")
    g.write("G1 X(($" + cur_tool + "x-$" + from_tool + "x-($" + cur_tool + "x_dif-$" + from_tool + "x_dif))) F" + str(air_travel_speed))
    g.write("G1 Y(($" + from_tool + "y-$" + cur_tool + "y+($" + from_tool + "y_dif-$" + cur_tool + "y_dif)))")
...
def print_inlet(up):
    ...
    g.toggle_pressure(com_ports[cur_tool_index]) # end extrusion of regular pluronic
...
for length_index in range(len(pressure_channel_lengths)):
    g.write("\n; length = "+str(pressure_channel_lengths[length_index])+".\n")
